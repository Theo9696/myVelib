...
public class UniformityBicycleConservation implements RidePreferences {
	...
	public UniformityBicycleConservation(){
		super();
	...
	public PlanningRide compute(ArrayList<Station> stations, User user, double[] destination, String typeBicycle) throws ComputingRideImpossibleException {
		...
		try {
			...
			for (Station s1 : stations) {
				...
				if ((s1.hasStationBicycle(typeBicycle) && s1.getNumberofBikeAvailable(typeBicycle)>departurenew.getNumberofBikeAvailable(typeBicycle) || distancesourcenew>distances1source) && 1.05*distancesource >= distances1source) {
					departurenew = s1;
				}
